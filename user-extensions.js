...
(function ($$) {
    ...
    function Logger() {
        ...
        this.trace = function (msg) {
            this.logit("debug", msg);
        }; // selenium doesn't have trace level
...
(function ($$) {
        ...
        evaluateXpath : function (doc, xpath, contextNode, resultType, namespaceResolver, resultObj) {
            ...
            try {
                var result = doc.evaluate(
                        xpath, contextNode || doc, namespaceResolver, resultType || XPathResult.ANY_TYPE, resultObj);
    ...
    ,
        fmtXpathResultType : function (result) {
            ...
            case result.ANY_UNORDERED_NODE_TYPE:
                return "uNODE " + result.singleNodeValue;
            case result.FIRST_ORDERED_NODE_TYPE:
                return "oNODE " + result.singleNodeValue;
            case result.UNORDERED_NODE_SNAPSHOT_TYPE:
                return result.snapshotLength + " uNODEs";
            case result.ORDERED_NODE_SNAPSHOT_TYPE:
                return result.snapshotLength + " oNODEs";
            case result.UNORDERED_NODE_ITERATOR_TYPE:
                return "uITR";
            case result.ORDERED_NODE_ITERATOR_TYPE:
                return "oITR";
            }
            return result;
        }
...
(function ($$) {
    ...
    ,
        spanSub : function (str, i) {
            if (i < str.length) {
                if (str.charAt(i) == "(")
                    return this.spanTo(str, i + 1, ")"); // recursively skip over intervening matching brackets
                else if (str.charAt(i) == "[")
                    return this.spanTo(str, i + 1, "]");
                else if (str.charAt(i) == "{")
                    return this.spanTo(str, i + 1, "}");
                else if (str.charAt(i) == "'")
                    return str.indexOf("'", i + 1); // no special meaning for intervening brackets
                else if (str.charAt(i) == '"')
...
(function ($$) {
    ...
    function idxHere() {
        return testCase.debugIndex;
        //return testCase.htmlTestCase.nextCommandRowIndex - 1;
    }
    ...
    function BlockDefs() {
        ...
        blkDefs.init = function (i, attrs) {
            ...
            LOG.info("SelBlocks: BlockDefs blkDefs[" + i + "].cmdName=" + blkDefs[i].cmdName);
            return blkDefs[i];
        };
        return blkDefs;
    }
    ...
    function blkDefAt(idx) {
        if (!blockDefs[idx]) {
            LOG.warn("No blockDefs defined at index " + idx);
        }
    ...
    function Stack() {
        ...
        stack.findEnclosing = function (_hasCriteria) {
    ...
    function setNextCommand(cmdIdx) {
        ...
        LOG.info("SelBlocks: setNextCommand: idx=" + testCase.htmlTestCase.nextCommandRowIndex);
    }
    ...
    $$.fn.interceptAfter(Selenium.prototype, "reset", function () {
        $$.LOG.trace("Selblocks In tail intercept :: Selenium.reset()");
        ...
        $$.LOG.debug("Replacing: testCase.nextCommand()");
        $$.fn.interceptReplace(testCase, "nextCommand", nextCommand);
    });
    ...
    function compileSelBlocks() {
        ...
        for (i = 0; i < testCase.commands.length; i++) {
            // LOG.info(JSON.stringify(testCase.commands[i]));
            if (testCase.commands[i].type === "command") {
                ...
                LOG.info("SelBlocks: CompileSelBlocks testCase.commands[" + i + "]: " + testCase.commands[i].command);
                ...
                case "if":
                    assertNotAndWaitSuffix(i);
                    lexStack.push(blockDefs.init(i, {
                            nature : "if",
                            elseIfIdxs : []
                        }));
                    break;
    ...
    function assertIntraBlockJumpRestriction(fromIdx, toIdx) {
        ...
        if (fromRange || toRange) {
            ...
            if (fromRange) {
                msg += " out of " + fromRange.desc + fromRange.fmt();
            }
            if (toRange) {
                msg += " into " + toRange.desc + toRange.fmt();
            }
    ...
    function findBlockRange(locusIdx) {
        ...
        for (idx = locusIdx - 1; idx >= 0; idx--) {
            ...
            if (blk) {
                ...
                switch (blk.nature) {
                case "loop":
                    return new CmdRange(blk.idx, blk.endIdx, blk.cmdName + " loop");
                case "function":
                    return new CmdRange(blk.idx, blk.endIdx, "function '" + blk.name + "'");
                case "try":
                    return isolateTcfRange(locusIdx, blk);
                }
            }
        }
    ...
    function isolateTcfRange(idx, tryDef) {
        ...
        var RANGES = [{
                ifr : tryDef.finallyIdx,
                ito : tryDef.endIdx,
                desc : "finally",
                desc2 : "end"
            }, {
                ifr : tryDef.catchIdx,
                ito : tryDef.finallyIdx,
                desc : "catch",
                desc2 : "finally"
            }, {
                ifr : tryDef.catchIdx,
    ...
    Selenium.prototype.doIf = function (condExpr, locator) {
        ...
        var ifState = {
            idx : idxHere(),
            elseIfItr : arrayIterator(ifDef.elseIfIdxs)
        };
    ...
    function cascadeElseIf(ifState, condExpr) {
        ...
        if (!evalWithVars(condExpr)) {
            ...
            if (ifState.elseIfItr.hasNext()) {
                setNextCommand(ifState.elseIfItr.next());
    ...
    Selenium.prototype.doTry = function (tryName) {
        ...
        if ($$.tcf.nestingLevel === 0) {
            ...
            $$.fn.interceptPush(editor.selDebugger.runner.IDETestLoop.prototype, "resume",
                $$.handleAsTryBlock, {
                handleError : handleCommandError
            });
        }
    ...
    function bubbleCommand(cmdIdx, _isContextBlockType) {
        ...
        $$.tcf.bubbling = {
            mode : "command",
            srcIdx : cmdIdx,
            _isStopCriteria : _isContextBlockType
        };
        if (hasUnspentFinally(tryState)) {
    ...
    Selenium.prototype.doFor = function (forSpec) {
        enterLoop(
            ...
            evalWithVars(loop.initStmt);
        } // initialize
        , function (loop) {
            return (evalWithVars(loop.condExpr));
        } // continue?
        , function (loop) {
            evalWithVars(loop.iterStmt);
        } // iterate
        );
    };
    ...
    Selenium.prototype.doForeach = function (varName, valueExpr) {
        ...
        , function (loop) {
            storedVars._i = loop.i;
            return (loop.i < loop.values.length);
        } // continue?
    ...
    function notifyFatalCmdRef(idx, msg) {
        notifyFatal(fmtCmdRef(idx) + msg);
    }
    function notifyFatalHere(msg) {
        notifyFatal(fmtCurCmd() + msg);
    }
    function assertCmd(idx, cond, msg) {
        if (!cond) {
            notifyFatalCmdRef(idx, msg);
        }
    }
    ...
    function assertRunning() {
        assert(testCase.started, " Command is only valid in a running script,"
    ...
    function IntervalTimer(msDuration) {
        ...
        this.getElapsed = function () {
            return (+new Date() - this.msStart);
        };
        this.hasExpired = function () {
            return (msDuration && this.getElapsed() > msDuration);
        };
        this.reset = function () {
            this.msStart = +new Date();
        };
